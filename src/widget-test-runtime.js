import { getBaseTestWidgetParams } from './widget-test-config'
...
export function createMbtcSwapTestWidget(target, options = {}) {
  const container = resolveContainer(target)

  if (!container) {
    throw new Error('MBTC test widget target container was not found.')
  }
  ...
  async function mountWidget() {
    ...
    widget = createCowSwapWidget(container, {
      params: getBaseTestWidgetParams(`${widgetWidth}px`, widgetHeight),
      provider,
      listeners,
    })
  }
  ...
  function refreshMeasurements() {
    ...
    if (widthChanged) {
      ...
      mountWidget().catch((error) => {
        console.error('Failed to remount MBTC test widget.', error)
      })
    }
  }
  ...
  mountWidget().catch((error) => {
    console.error('Failed to mount MBTC test widget.', error)
  })
